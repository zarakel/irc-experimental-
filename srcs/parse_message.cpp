	...
	{	
		if (Stock->line[0] == Stock->all_commands[i])  
		{
			if (Stock->line[1].size() > 0)
			{
				...
				&& Stock->line[1].empty() == 0
				&& Stock->line[2].empty() != 0
				&& (Stock->tmp_authentified[Stock->User] == 0 |
...
/* condition supposé de user :*/ if (Stock->line[0] == Stock->all_commands[2]
				&& Stock->line[1].empty() == 0
				&& Stock->line[2].empty() == 0
				&& Stock->line[3].empty() == 0
				&& Stock->line[4].empty() != 0
...
// une fois pass, nick et user de fait, authentified = 1
			...
			//	Stock->authentified[Stock->User] = 1;   ---   Usage temporaire
